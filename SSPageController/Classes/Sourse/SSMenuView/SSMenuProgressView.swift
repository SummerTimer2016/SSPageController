...
class SSMenuProgressView: UIView {
    /** 若image有值，优先使用image*/
    ...
    var cornerRadiusProgress: CGFloat = 1.5 {
        didSet {
            self.setNeedsDisplay()
        }
    }
...
//    fileprivate lazy var imageProgressView: UIImageView = {
//        let imageViewTemp:UIImageView = UIImageView.init()
...
//    }()
    ...
    override func draw(_ rect: CGRect) {
        ...
        let barframe = CGRect(x: startX, y: lineWidth / 2.0, width: width, height: height - lineWidth)
        let path: UIBezierPath = UIBezierPath(roundedRect:barframe , cornerRadius: self.cornerRadiusProgress)
        ...
        if (self.hasBorder) {
            ...
            let endX = last.maxX
            let path = UIBezierPath(roundedRect: CGRect(x: startX, y: lineWidth / 2.0, width: (endX - startX), height: height - lineWidth), cornerRadius: self.cornerRadiusProgress)
